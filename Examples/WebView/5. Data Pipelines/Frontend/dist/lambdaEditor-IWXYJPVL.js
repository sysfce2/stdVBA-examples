...
// src/lambdaCatalog.ts
var stdEnumeratorMembers = [
...
var resolveCompletionContext = () => ({ rowFields: [], joins: [] });
var editors = [];
...
function isDotAccessibleFieldName(name) {
  return /^[A-Za-z_$][\w$]*$/.test(name);
}
function formatFieldCompletionInsert(name) {
  if (isDotAccessibleFieldName(name)) return name;
  const escaped = name.replace(/"/g, '""');
  return `item("${escaped}")`;
}
function getJoinByAlias(joins, alias) {
...
function parseAccessExpression(prefix) {
...
function createReplaceRange(position, typed) {
  return {
    startLineNumber: position.lineNumber,
    endLineNumber: position.lineNumber,
...
function makeFieldSuggestion(monaco, fieldName, detail, range) {
...
function makeMethodSuggestion(monaco, label, detail, range) {
  return {
    label,
...
function resolveSuggestions(monaco, position, prefix) {
  ...
  const matchPrefix = (name) => name.toLowerCase().startsWith(typed.toLowerCase());
  const suggestions = [];
  const pushFieldSuggestions = (fields, detail) => {
...
async function getMonaco(getContext) {
  resolveCompletionContext = getContext;
  if (!monacoPromise) {
    monacoPromise = import("./editor.api-P6Y7XH7E.js").then((monaco2) => {
...
async function createLambdaEditor(container, value, onChange, getContext, options) {
